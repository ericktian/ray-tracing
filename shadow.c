...
int main(void)
{
   ...
   for( y = 0 ; y < N ; y++ )
   {
      for( x = 0 ; x < M ; x++)
      {
         ...
         double mint = INT_MAX; //placeholder for the minimum value of t
         int mincirc = 0;
         ...
         for(int circ = 0; circ<sizea; circ++){ //iterates through circles
        	...
        	if(mint==INT_MAX)continue;
				...
				printf("pxnew: %f\tpynew: %f\tpznew: %f\t\n", pxnew, pynew, pznew);
				printf("rxnew: %f\trynew: %f\trznew: %f\t\n", rxnew, rynew, rznew);
				printf("mint: %f\tmincirc: %i\t\n", mint, mincirc);
				
				for(int circ2 = 0; circ2<sizea; circ2++){ //iterates through circles to check which one crosses the light source
					//if (circ2==mincirc) continue;
					...
					double dznew = g.z-a[circ2].c.z;
					double dynew = g.y-a[circ2].c.y;
					double dxnew = g.x-a[circ2].c.x;
					...
					if( determinatenew >=0 ){	//-----FOUND A SOLUTION! So we make it in shadow-----
						...
						if(checkifpositive>0){
				  		 	...
				  		 	rgb[y][x][0] = a[mincirc].h.r * .5; //change color of pixel
							rgb[y][x][1] = a[mincirc].h.g * .5;
							rgb[y][x][2] = a[mincirc].h.b * .5;
							
							
							printf("circ2: %d\tmincirc: %d\tcheckifpositive: %f\n", circ2, mincirc, checkifpositive);
