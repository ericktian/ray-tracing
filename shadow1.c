         ...
         for(int circ = 0; circ<sizea; circ++){ //iterates through circles
             ...
             if( determinate >=0 ){
                  ...
                  if(t<=mint && t>0){
                     ...
                     */
                     //beer lambert thing
                 		double unnormalx = (px-a[circ].c.x);
                 		double unnormaly = (py-a[circ].c.y);
                 		double unnormalz = (pz-a[circ].c.z);
                		...
                		rgb[y][x][0] = a[circ].h.r * ((normalx*rx + normaly*ry + normalz*rz)*.5 + .5); //change color of pixel using dot product and formula
                     rgb[y][x][1] = a[circ].h.g * ((normalx*rx + normaly*ry + normalz*rz)*.5 + .5);
                     rgb[y][x][2] = a[circ].h.b * ((normalx*rx + normaly*ry + normalz*rz)*.5 + .5);
                     
               //      printf("normalx: %f\tnormaly: %f\tnormalz: %f\n", normalx, normaly, normalz);
