...
int main(void)
{
   ...
   //circles
  	...
  	//circles try number 2
  	for(int it = 0; it<1/*sizeof(a)/sizeof(a[0])*/; it++){
  		for(double theta = 0; theta<2*pi; theta+=.00001){ //increment through theta
  			int xval = (int)a[it].r*cos(theta);
  			int yval = (int)a[it].r*sin(theta);
			//printf("r: %f\tcx: %f\tcy: %f\ttheta: %f\n", a[it].r, a[it].c.x, a[it].c.y, theta);
			//printf("%i, %i\n", xval, yval);
			if(xval>=0&&xval<M && yval>=0&&yval <N){
				printf("r: %f\tcx: %f\tcy: %f\ttheta: %f\n", a[it].r, a[it].c.x, a[it].c.y, theta);
				printf("%i, %i\n", xval, yval);
			
				rgb[yval][xval][0] = 255;//a[it].h.r;//red1;
				rgb[yval][xval][1] = 255;//a[it].h.g;//green1;
				rgb[yval][xval][2] = 255;//a[it].h.b;//blue1;
